...
#if AP_DDS_GUIDED_SETPOINT_CTRL_ENABLED
ardupilot_msgs_msg_GuidedSetpoint AP_DDS_Client::rx_guided_setpoint_control_topic {};
#endif // AP_DDS_GUIDED_SETPOINT_CTRL_ENABLED
...
#endif // AP_DDS_GLOBAL_POS_CTRL_ENABLED
#if AP_DDS_GUIDED_SETPOINT_CTRL_ENABLED
    case topics[to_underlying(TopicIndex::GUIDED_SETPOINT_SUB)].dr_id.id: {
        const bool success = ardupilot_msgs_msg_GuidedSetpoint_deserialize_topic(ub, &rx_guided_setpoint_control_topic);
        if (success == false) {
            break;
        }

#if AP_EXTERNAL_CONTROL_ENABLED
        if (!AP_DDS_External_Control::handle_guided_setpoint(rx_guided_setpoint_control_topic)) {
            // TODO #23430 handle guided setpoint control failure through rosout, throttled.
        }
#endif // AP_EXTERNAL_CONTROL_ENABLED
        break;
    }
